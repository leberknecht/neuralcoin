...
class HasDroppedByOutputTypeTest extends \PHPUnit_Framework_TestCase
{
    /**
    ...
    {
        $this->outputType = new HasDroppedByOutputType();
    }
    ...
    public function testEvalutePrediction()
    {
        $prediction = new Prediction();
        $prediction->setPredictedValue(1);
        $prediction->setPriceAtPrediction(42.00);

        $prediction->setNetwork((new Network())->setOutputConfig((new OutputConfig())->setThresholdPercentage(-5.00)));
        $trade = new Trade();
        $trade->setPrice(35.00);
        $this->outputType->evaluatePrediction($prediction, $trade);

        $this->assertTrue($prediction->isDirectionHit());
    }
}
